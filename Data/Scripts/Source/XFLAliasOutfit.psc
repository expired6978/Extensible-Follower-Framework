Scriptname XFLAliasOutfit extends ReferenceAlias  
...
Function SaveEquipment()
	...
	While i < BipedObjects.length
		...
		if equippedForm && !OutfitHasArmor(equippedForm) ; Only save armor not part of the outfit
			BipedObjects[i] = equippedForm
			;Debug.Trace("Saving form: " + equippedForm + " to i: " + i + " slot: " + slot)
		else
			BipedObjects[i] = None
		endif
		slot *= 2
...
Function LoadEquipment()
	...
	While i < BipedObjects.length
		if BipedObjects[i]
			...
			If actorRef.GetItemCount(BipedObjects[i]) >= 1
